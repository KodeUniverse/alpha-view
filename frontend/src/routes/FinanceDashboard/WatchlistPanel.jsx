...
function WatchListItem({ symbol }) {
    ...
    useEffect(() => {
        ...
        const fetchStockData = async () => {
            ...
            try {
                const res = await fetch(`${import.meta.env.API_URL}/stock-data/hist-ts/${symbol}/latest`)
                if (!res.ok) {
    ...
    return (<>
        <ListItemButton sx={{ height: "15%", display: "flex", justifyContent: "center", gap: 2 }}>
            <Typography sx={{ fontWeight: 700 }}>{symbol}</Typography>
            <StockChart data={stockData} chartType="area" containerStyles={{ width: "50%", height: "100%" }} />
